...
// fund returns a state where alice already holds `amount` coins.
...
func TestApplyMovesCoins(t *testing.T) {
	s := fund(t, "alice", 100)
	if err := s.Apply(NewTransfer("alice", "bob", 30, 2)); err != nil {
		t.Fatalf("a valid transfer must succeed: %v", err)
	}
	if got := s.Balance("alice"); got != 70 {
		t.Errorf("alice = %d, want 70", got)
	}
	if got := s.Balance("bob"); got != 30 {
...
func TestApplyRejectsOverspendingWithoutChangingBalances(t *testing.T) {
	...
	err := s.Apply(NewTransfer("alice", "bob", 150, 2))
	if err == nil {
...
func TestApplyRejectsMalformedAndOverflow(t *testing.T) {
	s := fund(t, "alice", 100)
	if err := s.Apply(NewTransfer("alice", "bob", -1, 2)); err == nil {
		t.Error("a negative amount must be rejected")
	}
	if err := s.Apply(NewTransfer("nobody", "bob", 1, 2)); err == nil {
		t.Error("an account with no coins cannot send any")
	}

	rich := fund(t, "whale", math.MaxInt64)
	if err := rich.Apply(NewCoinbase("whale", 1, 3)); err == nil {
		t.Error("a credit that would overflow int64 must be rejected")
	}
}

// A batch is atomic: if the third transaction fails, the first two must not
// have moved anything either.
func TestApplyAllIsAtomic(t *testing.T) {
	s := fund(t, "alice", 100)
	batch := []Transaction{
		NewTransfer("alice", "bob", 40, 2),
		NewTransfer("bob", "carol", 40, 3), // fine: bob was paid a line earlier
		NewTransfer("alice", "carol", 999, 4),
	}
...
func TestAccountsAreSortedAndCloneIsIndependent(t *testing.T) {
	...
	clone := s.Clone()
	if err := clone.Apply(NewTransfer("zoe", "adam", 10, 3)); err != nil {
		t.Fatal(err)
	}
